...
public class AccountActivateActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mPrepareDocumentsTextView.setOnClickListener(new OnClickListener() {			
			...
			public void onClick(View v) {
				DataBaseManager.getInstance(AccountActivateActivity.this).setAccountStatus(DataBaseManager.ACCOUNT_STATUS_DOCUMENTS);
				Intent intent = new Intent(getBaseContext(), AccountOpenActivity.class);
		...
		mChatImageView.setOnClickListener(new OnClickListener() {			
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(AccountActivateActivity.this, ChatActivity.class);
				startActivity(intent);
			}
		});
	}
					...
					if (resultCode == 200) {
						...
						if (DataBaseManager.ACCOUNT_STATUS_DOCUMENTS.equals(organization.getString("status"))) {
							...
							if (!organization.isNull("account_number")) {
								SharedPreferenceManager.getInstance(AccountActivateActivity.this).setAccountNumber(organization.getString("account_number"));
							}
							if (!organization.isNull("account_creds")) {
								SharedPreferenceManager.getInstance(AccountActivateActivity.this).setCreds(organization.getString("account_creds"));
							}
							if (!resultObject.isNull("dbo_user")) {
								...
								if (!dboUser.isNull("is_signature_created")) {
									SharedPreferenceManager.getInstance(AccountActivateActivity.this).setSignatureCreated(dboUser.getBoolean("is_signature_created"));
								}
							}
